...
export const listProducts = (keyword = '', pageNumber = '', category = '') => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${API_URL}/api/products?keyword=${keyword}&pageNumber=${pageNumber}&category=${category}`
    );
...
export const listAllProducts = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`${API_URL}/api/products/all`, config);
    dispatch({ type: PRODUCT_LIST_SUCCESS, payload: { products: data, pages: 1, page: 1 } });
  } catch (error) {
